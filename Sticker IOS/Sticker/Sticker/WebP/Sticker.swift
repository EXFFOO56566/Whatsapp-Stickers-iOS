...
class Sticker {
    ...
    init(contentsOfUrl filename: String, emojis: [String]?) throws {
        
        // check if file present in local directory
        
        // else
        let forecastURL1 = NSURL(string: filename)
        let testImage1 = try Data(contentsOf: forecastURL1! as URL)
        //can save to document
        self.imageData = try ImageData.imageDataIfCompliant(rawData: testImage1, extensionType: ImageDataExtension.webp, isTray: false)
        //
